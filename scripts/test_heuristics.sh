...
FAST_DOWNWARD_DIR="../fast-downward"
...
declare -a searches=( "lazy(tiebreaking([$MY_H,$ADD_H,$CPDB_GEN_H]),max_time=$time_limit)" \
"lazy_greedy([$CPDB_H,$FF_H],max_time=$time_limit)" 
"lazy_greedy([$CPDB_GEN_H,$FF_H],max_time=$time_limit)"
...
for search in ${searches[@]}; do
    ...
    for ((i=0;i<${#domains[@]};++i)); do
        ...
        $PROBLEMS_DIR/${domains[i]} \
        $PROBLEMS_DIR/${problems[i]} \
                                          --search "$search" \
